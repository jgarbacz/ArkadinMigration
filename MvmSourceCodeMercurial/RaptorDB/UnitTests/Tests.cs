...
namespace UnitTests
{
    ...
    public class Tests
    {
        ...
        static string _Path = @"c:\RaptorDBTest\";

        [Test]
        public static void Enumerate()
        {
            var db = new RaptorDB<Guid>(_Path + "enumerate", true);
        ...
        {
            var db = new RaptorDB<Guid>(_Path + "duptestfetch", true);
            int guidcount = 1000;
            int dupcount = 100;
        ...
        public static void Multithread_test()
        {
            ...
            var db = RaptorDB.RaptorDB<Guid>.Open(_Path + "multithread", false);
        ...
        private static void Set_Get(string fname, int count, bool shutdown, bool skiplist)
        {
            ...
            var db = RaptorDB.RaptorDB<Guid>.Open(_Path + fname, false);
            ...
            if (shutdown == true)
            {
                db.Dispose();
                db = null;
                db = RaptorDB.RaptorDB<Guid>.Open(_Path + fname, false);
            }
            GC.Collect(2);
            int notfound = 0;
            c = 0;
        ...
        {
            RaptorDB.RaptorDB<long> rdb = new RaptorDB.RaptorDB<long>(_Path + "remove.dat", false);
            rdb.Set(1, "a");
            rdb.Set(2, "b");
            rdb.Shutdown();


            rdb = new RaptorDB.RaptorDB<long>(_Path + "remove.dat", false);
            rdb.RemoveKey(1);
            rdb.Shutdown();
        ...
        {
            var db = RaptorDB<string>.Open(_Path + "strings", 255, true);
            for (int i = 0; i < 100000; i++)
            {
                db.Set("asdfasd" + i, "" + i);
            }
        ...
        public static void RaptorDBString_test()
        {
            ...
            var rap = new RaptorDBString(_Path + "longstringkey", false);
        ...
        private static void Optimized_GUID(int count)
        {
            ...
            var db = new RaptorDBGuid(_Path + count);
            ...
            for (int i = 0; i < count; i++)
                guids.Add(Guid.NewGuid());
            Console.WriteLine("done");
            FileStream fs = new FileStream(_Path + "guids", FileMode.Create);
            for (int i = 0; i < count; i++)
