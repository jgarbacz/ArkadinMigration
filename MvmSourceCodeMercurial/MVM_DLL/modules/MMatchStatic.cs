...
namespace MVM
{
    ...
    class MMatchStatic : IModuleSetup, IModuleRun
    {
        // from xml
        ...
        {
            MMatchStatic m = new MMatchStatic();
            // xml extraction
            ...
            m.pattern = m.regexParsed.Read(mc);
            if (m.caseParsed.Read(mc).Equals("1"))
            {
                m.regex = new Regex(m.pattern,RegexOptions.Compiled|RegexOptions.IgnoreCase );
            }
            else
            {
                m.regex = new Regex(m.pattern,RegexOptions.Compiled);
            }
            // runtime
            run.Add(m);
        }
