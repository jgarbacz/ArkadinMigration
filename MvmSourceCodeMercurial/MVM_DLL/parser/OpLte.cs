...
namespace MVM
{
    public class OpLte : BaseBinaryOpSetup
    {
        public override IReadString CreateRun(IReadString left, IReadString right)
        {
            return new OpLteRun(left, right);
        }
    }

    public class OpLteRun : ReadStringBase
    {
            private readonly IReadString left;
            private readonly IReadString right;
            public OpLteRun(IReadString left, IReadString right)
            {
                this.left = left;
                this.right = right;
            }
            public override string Read(ModuleContext mc)
            {
                ...
                return left.IsNumericLte(right) ? "1" : "0";
            }
    }
}
