...
absoluteLocationPathNoroot
  ...
  | '/@' nodeTest (traverse)*-> ^(ABSOLUTE_PATH ^(NAME_MATCH nodeTest) (traverse)*)
  | '/'  step (traverse)* ->^(ABSOLUTE_PATH ^(MATCH step) (traverse)*)
  | '//@' nodeTest (traverse)*-> ^(ABSOLUTE_PATH ^(RECURSIVE_NAME_MATCH nodeTest) (traverse)*)
...
traverse
	: '/..' -> ^(TRAVERSE_UP)
	| '/.' -> ^(CURRENT_NODE)
	| '/@' nodeTest-> ^(NAME_MATCH nodeTest)
	| '/' step -> ^(MATCH step)
	| '//@' nodeTest-> ^(RECURSIVE_NAME_MATCH nodeTest)
